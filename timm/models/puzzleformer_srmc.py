...
@register_model
def puzzleformer_srmc_tiny_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=192, depth=12, num_heads=3, class_token=True)
    model = _create_vision_transformer('puzzleformer_srmc_tiny_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def puzzleformer_srmc_small_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Small (Vit-S/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=384, depth=12, num_heads=6, class_token=True)
    model = _create_vision_transformer('puzzleformer_srmc_small_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def puzzleformer_srmc_base_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Base (Vit-B/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=768, depth=12, num_heads=12, class_token=True)
    model = _create_vision_transformer('puzzleformer_srmc_base_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
