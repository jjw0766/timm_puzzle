...
class PuzzleTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.mask_vector = nn.Parameter(torch.zeros(1, 3, 1, 1, **dd))
    ...
    def init_weights(self, mode: str = '') -> None:
        ...
        if self.mask_vector is not None:
            nn.init.normal_(self.mask_vector, std=1e-6)
    ...
    def forward_features_pretrain(self, x: torch.Tensor, piece_size: int = None, attn_mask: Optional[torch.Tensor] = None, masks: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Forward pass through feature layers (embeddings, transformer blocks, post-transformer norm)."""
        x = x.masked_fill(masks.unsqueeze(1).expand(-1,3,-1,-1), self.mask_vector)
    ...
    def forward_pretrain(self, x: torch.Tensor, piece_size: int = None, attn_mask: Optional[torch.Tensor] = None, masks: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        x = self.forward_features_pretrain(x, piece_size, attn_mask=attn_mask, masks=masks)
...
@register_model
def puzzleformer_srm2_tiny_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=192, depth=12, num_heads=3, class_token=True)
    model = _create_vision_transformer('puzzleformer_srm2_tiny_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def puzzleformer_srm2_small_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Small (Vit-S/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=384, depth=12, num_heads=6, class_token=True)
    model = _create_vision_transformer('puzzleformer_srm2_small_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def puzzleformer_srm2_base_patch16_256(pretrained: bool = False, **kwargs) -> PuzzleTransformer:
    """ ViT-Base (Vit-B/16)
    """
    model_args = dict(patch_size=16, piece_sizes=(16, 32, 64, 128), embed_dim=768, depth=12, num_heads=12, class_token=True)
    model = _create_vision_transformer('puzzleformer_srm2_base_patch16_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
